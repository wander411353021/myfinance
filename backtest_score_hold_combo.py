# -*- coding: utf-8 -*-
"""
评分 × 持有期 组合策略回测 — 穿越牛熊验证
假设：
  高分信号（甜点区42+）趋势延续性强 → 持有60天吃趋势
  低分信号（<42）反弹动能弱 → 持有20天快进快出
对比：纯20天、纯60天、按评分分档持有
"""
import sys
sys.path.insert(0, '.')
...
print("\n" + "=" * 90)
print("组合策略：高分持有60天 + 低分持有20天")
print("=" * 90)
strategies = [
    ('纯20天', lambda s: s['ret20']),
    ('纯60天', lambda s: s['ret60']),
    ('组合(40分分档)', lambda s: combo(s, 40)),
    ('组合(42分分档)', lambda s: combo(s, 42)),
    ('组合(45分分档)', lambda s: combo(s, 45)),
    ('组合(48分分档)', lambda s: combo(s, 48)),
    ('组合(50分分档)', lambda s: combo(s, 50)),
]
print(f"{'策略':<22} {'信号数':>6} {'胜率':>8} {'均值':>8} {'中位':>8}")
print("-" * 65)
for label, fn in strategies:
    rets = [fn(s) for s in all_signals]
    n, wr, mean, med = calc_stats(rets)
    print(f"{label:<22} {n:>6} {wr:>7.1%} {mean:>7.1%} {med:>7.1%}")
...
# 分档明细：40分上下各持有期
print("\n" + "=" * 90)
print("甜点区(40+) vs 低分(<40) 不同持有期对比")
print("=" * 90)
hi = [s for s in all_signals if s['score'] >= 40]
lo = [s for s in all_signals if s['score'] < 40]
print(f"\n高分信号(>=40): {len(hi)}个")
for hd_name, key in [('20天','ret20'), ('60天','ret60')]:
    rets = [s[key] for s in hi]
    n, wr, mean, med = calc_stats(rets)
    print(f"  {hd_name}: 胜率{wr:.1%} 均值{mean:.1%}")
print(f"\n低分信号(<40): {len(lo)}个")
for hd_name, key in [('20天','ret20'), ('60天','ret60')]:
    rets = [s[key] for s in lo]
    n, wr, mean, med = calc_stats(rets)
    print(f"  {hd_name}: 胜率{wr:.1%} 均值{mean:.1%}")
