...
- **股眼标注(aben_patterns.py detect_guyan v2)叠加在 volume 面板**(2026-08-27 用户要求与 volume 合并):
  - 色块 + 柱子上方类型文字:reversal 橙(堆前60日跌>8%+后止跌) / breakout 蓝(横盘→突破) / accel 紫(上涨加速) / mega_absorb 红(峰值量比>8 吸筹)
  - 股眼=改变走势的关键量堆(用户定义),定位长期观察非买入信号;详见 aben-arch skill
- Volume 高度演进:1.4 → 0.7(压半)→ 合并股眼后 1.0 → 1.3(2026-08-27 调高)
...
## V10 金色高胜率坑标记(2026-08-27)
- 判定:reg250 20日斜率>0(上行) OR 放量堆确认(出坑后7天峰值≥5)= 96% 组(方案 C,224/588)
- K线面板:坑区间金色背景带(#FFD54F alpha 0.22);GOLD PIT 面板:高胜率坑色块金色(#FFC107 alpha 0.95)
- 所有坑保留(质量色不变),金色=可买区
- reg 线分段着色:reg250 粗线(绿#2E7D32/红#C62828)+ reg120 细线(深绿#1B5E20/深红#B71C1C),20日斜率

## V10 坑检测与金色标记定版(2026-08-28)
- **坑检测 = detect_golden_pit_v3**(z 连续 3 日 >= -1.5 确认出坑,假出坑不切坑;无未来函数,全池自检 0 不一致)
- **金色标记(73.3% 档)**: 距250日高点<-40% + 坑长<15 + 90天内第<=2次出坑
  - 高收益坑特征: 低位(距高点-40%) + 短坑(<15) + 前1-2次出坑(均值+14.6%, 高收益占42%)
- 废弃: 旧金色=坑长>=6+reg上行(64%, 弱因子) / 更早 96%(reg上行OR放量堆, 未来函数假象)
- 坑检测演进: detect_golden_pit(豆包修复) → v2(出坑即结算, 有假出坑切坑) → v3(定版)

## V10 双基准接入(2026-08-28)
- **坑检测**: detect_golden_pit_v3(_fc, _frg2, _frg120, use_dual=True)
  - _frg2 = reg_preds_long(250日)全量; _frg120 = reg_preds(120日)全量(缺失时自算)
  - reg120 > reg250 时(牛市/强势)用 reg120 做 z 基准——补牛市信号(价格跌破 reg120 也成坑)
  - 1000池: 信号 3494→5492(+57%), 20日胜率 58.4%(不降反升)
- **reg 平滑**: smooth_w=5 参数(默认), 验证无变化(回归线本身平滑), 保留无副作用
- **历史**: detect_golden_pit → 豆包修复 → v2 → v3(z确认+min_depth=8%)→ v3+双基准(当前)

## Turn-Up Target 坑内突破分析(2026-08-29 polo4111)
- **信号源**: 黄金坑坑段[s,lch]内, 价格突破 Turn-Up Target 线(compute_turn_positive_prices)的事件
- **核心区分: 坑底前突破 vs 坑底后突破**
  - 坑底前突破(bi<=b, 还在下跌中碰target): 胜率21%, r20=-6.6%, **负期望, 坚决回避**(下跌中继的假反弹)
  - 坑底后突破(bi>b, 见底反弹后碰target): 胜率53%, r20=+2.7%
- **甜点: 突破价距坑底涨幅 0~10%**
  - 0~3%: 胜率71%(n=35, 样本少), r20=+8.6%
  - 3~6%: 胜率64%, r20=+3.6%
  - 6~10%: 胜率58%, r20=+3.7%
  - 10~15%: 胜率55%
  - **>25%(追高): 胜率39%, r20=-1.4%, 负期望**
- **真突破确认(10日内target线不重现/reenter)**: 坑底后+真突破 = 胜率61%, r20=+5.3%
- **最佳组合(即时可执行)**: 坑底后 + 距坑底3~10% + 真突破确认 = 胜率63%, r10胜率69%, n=737
- **坑长分层**: 坑长16~30天最好(胜率59%), 坑长>60天差(胜率39%)
- **实现脚本**: pit_target_break_analysis.py(全池验证)

## 高收益坑内突破特征(2026-08-29 polo4111)
- **定义**: 突破后60日收益 > +15% = 高收益, 占比25.6%
- **核心结论: 高收益是肥尾, 常规技术特征无法预测**
  - 高收益组 vs 普通组在坑深/坑长/突破位置/z/量能上差异极小
  - Top案例r60普遍+150%以上(题材/重组/大盘风口驱动), 技术特征五花八门
  - 300251(光线传媒)2024-09-25案例: 深跌-44%+坑长68天+5日反弹14%+真突破, r60=+22.6%
- **极端组合可提高概率(从25%到39%)**:
  - 深跌<-40% + 长坑>40天 + 强反弹(5日>8%) + 真突破 = 高收益占比39.2%, r60均值+9.9%(整体2倍), n=102
  - 信号量少(约每月3个), 但质量高
- **剩余61%仍是普通收益**: 需配合动态离场(止盈兑现反弹)提高整体期望
- **实现脚本**: high_return_feature_analysis.py(全池验证)
- **教训**: 不要追求"精准预测高收益"(做不到, 肥尾+外部驱动); 用极端组合筛选提高概率即可

## V10 reg平滑画图(2026-08-29 polo4111)
- **问题**: reg120/reg250线有毛刺, 用户要求更平滑
- **方案**: 对reg做滚动平均平滑(smooth_w=10日), 画图和坑检测都用平滑后reg(保持一致)
