...
- `panic_reversal.py`:
  ...
  - `compute_strength(closes, highs, lows, k=2.0, alpha=2.0, m=30.0, atr=None, win=10, dir_atr=2.0, reg_preds=None, confirm_flip=2, flip_strong=0.08, min_main=3, decay_days=7, decay_factor=0.75)`
...
回归线门控: reg_preds 提供且 close < reg 时,正柱(反转)不显示——
...
死区日   = 延续前方向衰减: cur*|last_s|*decay_factor^k (decay_days=7, factor=0.75),
          衰减结束归零(无最小值保底)
```
...
# streamlit: streamlit run streamlit_demo.py(走 run_segmentation,默认 strength_win=10 dir_atr=2.0 decay_days=7)
