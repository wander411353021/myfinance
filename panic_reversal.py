    ...
    for s0, e0 in merged:
        ...
        for p in seg:
            if m2 and p[0] - m2[-1][1] <= merge_gap:
                ...
                m2.append(list(p))
        s, e = m2[0][0], m2[-1][1]
        # ── 因果修复(2026-08-28 polo4111 深度截断自检)────────────────────────
        # 旧版 b=argmin(closes[s:e+1]) 用完整段(可延伸至出坑日之后很远)定坑底,
        # 当"出坑后价格又创新低"时(如 sh600493 471底→472出坑→482再创新低),
        # b 被未来数据更新、出坑判定参照 closes[b]*1.02 变高 → lch 被推迟到
        # 503(实盘 472 即确认),回测信号与实盘截断可复现信号不一致 = 未来函数。
        # 修复: 从段起点 s+1 线性扫描,实时维护"截至当天的段内最低"b(只用<=当前日
        # 历史),出坑判定参照"当天已知最低",则 b 与 lch 均只用 <=lch 数据确定,
        # 与实盘"当日可见信息"完全一致。若当日创新低则 b 更新且不会触发出坑。
        # ─────────────────────────────────────────────────────────────────────
        b = s
        lch = None
        for i in range(s + 1, n):
            if closes[i] < closes[b]:
                b = i
            # 出坑必须"走出坑":收复门控线 且 收盘高于"当天已知坑底"价≥2%
            # (否则弱反弹/假出坑——603515 +0.1%/600406 +0.4%/002039 +0.2%
            #  刚过线小阳线不算出坑;验证超高信号 93.1%→93.2% 不降)
...
def detect_grid_break(closes, reg120, reg250,
    ...
    以 max(reg120, reg250) 为基准, 只取向上平移档位(>=+5%, 用户要求不做向下档),
    某条格栅线"长期压制"股价(close 在其下方连续 >= min_suppress 天),
    向上突破(收盘站上该线)后 = 信号。

    验证(1000池, 20日): 上方档整体 50-52%(观察面板定位, 不作独立买入信号)。
    全部因果(只用<=突破日数据)。
